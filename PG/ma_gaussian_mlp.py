...
# -*- coding: utf-8 -*-
"""
Created on Sat May 19 13:35:03 2018
...
class MLP:
    def __init__(self, N,
        ...
        self.n = (N+1)  # number of states
        self.m = 1  # number of actions
    ...
    def mean_LL(self, i, observations, actions, model=None, log_std=None):
        ...
        LL = - 0.5 * torch.sum(zs ** 2, dim=1) + \
             - torch.sum(self.log_std) + \
             - 0.5 * self.m * np.log(2 * np.pi)
        '''
        LL = - 0.5 * torch.sum(zs ** 2, dim=1) + \
             - torch.sum(policy[i].old_log_std) + \
             - 0.5 * policy[i].m * np.log(2 * np.pi)
        LL = - 0.5 * torch.sum(zs ** 2, dim=1) + \
             - torch.sum(policy[i].log_std) + \
             - 0.5 * policy[i].m * np.log(2 * np.pi)
        '''
        return mean, LL
    ...
    def old_dist_info(self, i, observations, actions):
        mean, LL = self.mean_LL(i, observations, actions, self.old_model, self.old_log_std)
        #mean, LL = policy[i].mean_LL(i, observations, actions, policy[i].old_model, policy[i].old_log_std)
        return [LL, mean, self.old_log_std]
        #return [LL, mean, policy[i].old_log_std]
    
    def new_dist_info(self, i, observations, actions):
        mean, LL = self.mean_LL(i, observations, actions, self.model, self.log_std)
        #mean, LL = policy[i].mean_LL(i, observations, actions, policy[i].model, policy[i].log_std)
        return [LL, mean, self.log_std]
        #return [LL, mean, policy[i].log_std]
