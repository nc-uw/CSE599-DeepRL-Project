...
class LinearBaseline:
    def __init__(self, N, reg_coeff_init=1e-5):
        self.N = N
        self.n = N+1
        ...
        self._coeffs = {}
        for i in range(N):
            self._coeffs[self.variables[i]] = None
            self._reg_coeff[self.variables[i]] = reg_coeff_init
    ...
    def fit(self, paths, return_errors=False):
        ...
        if return_errors:
            ...
            error_before = {}
            for i in range(self.N):
                predictions[self.variables[i]] = featmat[self.variables[i]].dot(self._coeffs[self.variables[i]]) if self._coeffs[self.variables[i]] is not None else np.zeros(returns[self.variables[i]].shape)
        ...
        for i in range(self.N):
            for _ in range(10):
                ...
                self._coeffs[self.variables[i]] = np.linalg.lstsq(
                ...
                if not np.any(np.isnan(self._coeffs[self.variables[i]])):
                    break
    ...
    def predict(self, path):
        predictions = {}        
        for i in range(self.N):
            if self._coeffs[self.variables[i]] is None:
                predictions[self.variables[i]] = np.zeros(len(path["rewards"][self.variables[i]]))
            else:
                predictions[self.variables[i]] = self._features(path)[self.variables[i]].dot(self._coeffs[self.variables[i]])
        return predictions